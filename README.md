...
| A single mis-named profile like `Claude (Work,Personal)` got created, and both accounts end up sharing it | You ran `Setup.ps1` via an external `powershell -File Setup.ps1 -Profile Work,Personal` (nested process, cmd.exe, Explorer's Run dialog, etc.) instead of calling it directly from an open PowerShell prompt (`.\scripts\Setup.ps1 -Profile Work,Personal`) — that invocation style doesn't split the comma-separated value into an array, so it's treated as one profile name. Current versions of `Setup.ps1` split it automatically either way, so just re-run it; delete the stray `Claude (Work,Personal).lnk` shortcut afterwards. |
